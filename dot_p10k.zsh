# From the one disktro of linux I have tried (ubuntu server) when at the tty console it sets $TERM to "linux".
# When in the tty console ttf fonts and more than 8 colors are not supported. So to allow for a single file and 
# keep my preferances while work on both tty console and a more full featured terminal.
if [[ $TERM = "linux" ]] ; then
  is_console=true
else
  is_console=false
...
# The is_console=true was
# Generated by Powerlevel10k configuration wizard on 2021-08-06 at 14:53 CDT.
...
# The is_console=false was
# Generated by Powerlevel10k configuration wizard on 2021-08-08 at 18:34 CDT.
# Based on romkatv/powerlevel10k/config/p10k-lean-8colors.zsh, checksum 03079.
# Wizard options: ascii, lean_8colors, 12h time, 2 lines, solid, blue-ornaments, sparse,
# concise, instant_prompt=verbose.
# Type `p10k configure` to generate another config.
#
# Config for Powerlevel10k with classic powerline (is_console=false) and 8-color lean (is_console=true) prompt style. Type `p10k configure` to generate
...
() {
  ...
  # Example output (console): master wip <42>42 *42 merge ~42 +42 !42 ?42.
  #
